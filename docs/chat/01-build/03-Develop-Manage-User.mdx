---
id: docs-chat-develop-manage-user
title: Manage User
hide_title: true
slug: ./manage-user
displayed_sidebar: pushChatSidebar
sidebar_position: 3
image: "/assets/docs/previews/docs_chat_develop--manage_user.png"
---
...
<head>
  <title>Manage User | Push Chat | Push Documentation</title>
</head>
...
| -          | `options.overrideAccount`            | -                                  | -     | The [account](/docs/chat/supported-wallet-standards "Push Chat support wallet standards") for which info is retrieved, can override to get info of other accounts not owned by the user. If not provided, it is derived from signer. |
...
const response = await userAlice.profile.update({
  name: 'Alice',
  desc: 'Alice is a software developer',
  picture: 'data:image/png;base64,' // base64 encoded image
});
```
...
const walletAlice = await userAlice.encryption.update(CONSTANTS.USER.ENCRYPTION_TYPE.PGP_V3);
...
const nftAlice = await userAlice.encryption.update(CONSTANTS.USER.ENCRYPTION_TYPE.NFTPGP_V1, {
  versionMeta: {
    NFTPGP_V1: {
...
:::tip
Attaching progress hook functionality in [Initialize user API](/docs/chat/build/initialize-user) will help you to track the progress of encryption update.
:::
